...
class CompositionService:
    ...
    def compose(
        ...
        logger.info(
            f"Successfully placed {len(placed)} out of {len(subjects)} subjects"
        )

        if not placed:
            logger.warning(
                "No subjects could be placed on canvas - creating wordcloud-only collage"
            )
    ...
    def _place_subjects(
        ...
        for i, subject in enumerate(subjects):
            ...
            if w > W or h > H:
                logger.warning(
                    f"Subject {i} ({w}x{h}) larger than canvas ({W}x{H}) - discarding"
                )
                discarded_count += 1
                continue
            ...
            while self._has_collisions(rect, rectangles):
                ...
                if attempts > _PLACEMENT_ATTEMPT_CAP:
                    logger.warning(
                        f"Subject {i} could not be placed after {_PLACEMENT_ATTEMPT_CAP} attempts"
                    )
                    rect = None
                    discarded_count += 1
                    break
            ...
            if rect is not None:
                ...
                logger.debug(
                    f"Placed subject {i} at {rect.x1},{rect.y1} after {attempts} attempts"
                )
        ...
        for i, (rect, subject) in enumerate(zip(rectangles, subjects)):
            try:
                ...
                if x1 < 0 or y1 < 0 or x2 > bg_w or y2 > bg_h:
                    logger.warning(
                        f"Subject {i} bounds ({x1},{y1},{x2},{y2}) exceed background ({bg_w},{bg_h})"
                    )
                    continue
