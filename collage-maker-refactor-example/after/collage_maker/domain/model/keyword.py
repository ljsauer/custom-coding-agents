...
class Keyword(BaseModel):
    ...
    frequency: int = Field(default=1, description="Frequency of this keyword in source text", ge=1)
    confidence: float = Field(default=1.0, description="Confidence score for keyword relevance", ge=0.0, le=1.0)
    ...
    def get_search_variants(self) -> list[str]:
        ...
        if self.text.endswith('s') and len(self.text) > 3:
            # Try singular
