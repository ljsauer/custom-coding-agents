...
class GoogleImageFetcher(IReferenceImageSource):
    ...
    def fetch_for_keyword(
        ...
        try:
            ...
            logger.info("Fetched %d images for keyword '%s'", len(unique_results), keyword.text)
            return unique_results
            
        except Exception as e:
            logger.error("Failed to fetch images for keyword '%s': %s", keyword.text, e)
            return []
    ...
    def _scrape_images(
        ...
        try:
            ...
            logger.warning("Failed to scrape images for '%s': %s", search_term, e)
            return []

    def _extract_images(self, soup: BeautifulSoup, min_quality: ImageQuality) -> list[ImageResult]:
        ...
        for img_tag in soup.find_all("img"):
            ...
            try:
                ...
                logger.debug("Failed to process image %s: %s", src, e)
                continue
