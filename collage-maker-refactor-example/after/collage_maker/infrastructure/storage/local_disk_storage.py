...
class LocalDiskStorage(ICollageStorage):
    ...
    def save(self, collage_id: str, image_bytes: bytes) -> str:
        ...
        Raises:
            ValueError: If collage_id is invalid
    ...
    def _ensure_directory_exists(self) -> None:
        ...
        try:
            ...
            raise OSError(f"Cannot create collage directory {self._collage_dir}: {e}") from e
    ...
    def _validate_collage_id(self, collage_id: str) -> None:
        ...
        if len(collage_id) > _MAX_FILENAME_LENGTH - len(_EXTENSION):
            raise ValueError(f"Collage ID too long (max {_MAX_FILENAME_LENGTH - len(_EXTENSION)} chars)")
        
        # Check for filesystem-unsafe characters
        unsafe_chars = {'/', '\\', ':', '*', '?', '"', '<', '>', '|', '\0'}
        if any(char in collage_id for char in unsafe_chars):
            raise ValueError(f"Collage ID contains unsafe characters: {unsafe_chars & set(collage_id)}")
