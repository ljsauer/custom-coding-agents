...
class CreateCollageUseCase:
    ...
    def execute(self, text: str, collage_name: str | None = None) -> Collage:
        ...
        try:
            ...
            logger.info(f"Successfully processed {len(subjects)} subject images")

            # 3. Compose the collage
            logger.info(f"Composing collage with {len(subjects)} subjects")
            ...
            if len(image_bytes) < 1024:  # Sanity check
                raise CollageCreationError(
                    "Generated image is too small - composition may have failed."
                )

            # 5. Calculate processing metrics
            processing_time = time.time() - start_time
            logger.info(f"Collage processing completed in {processing_time} seconds")
            ...
            collage.add_metadata(
                image_count=len(subjects), processing_time=processing_time
            )

            # 7. Persist to repository first (for referential integrity)
            self._repository.save(collage)
            logger.info(
                f"Saved collage to repository with ID: {collage.id}",
            )
            ...
            logger.error(
                "Unexpected error during collage creation: %s", e, exc_info=True
            )
            raise CollageCreationError(
                f"Collage creation failed due to an unexpected error: {e}"
            )
    ...
    def _collect_subjects(self, keywords: list[Keyword]) -> list[np.ndarray]:
        ...
        for i, keyword in enumerate(keywords):
            logger.info(
                "Processing keyword %d/%d: '%s'", i + 1, len(keywords), keyword.text
            )
            ...
            try:
                ...
                if not image_results:
                    logger.info("No images found for keyword '%s'", keyword.text)
                    continue
                ...
                for result in image_results:
                    ...
                    if subject is not None:
                        ...
                        logger.info(
                            "Successfully processed subject from '%s'", keyword.text
                        )
                    else:
                        skipped_images += 1
                        logger.info(
                            "Could not isolate subject from image for '%s'",
                            keyword.text,
                        )

            except Exception as e:
                logger.warning(
                    "Failed to process images for keyword '%s': %s", keyword.text, e
                )
                continue

        logger.info(
            "Image processing complete: %d processed, %d subjects extracted, %d skipped",
            processed_images,
            len(subjects),
            skipped_images,
        )
    ...
    def _decode_image(raw: bytes) -> np.ndarray | None:
        ...
        try:
            ...
            logger.info("Failed to decode image: %s", e)
            return None
