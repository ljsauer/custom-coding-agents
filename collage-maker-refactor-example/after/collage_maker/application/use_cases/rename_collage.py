# application/use_cases/rename_collage.py
#
...
class RenameCollageUseCase:
    ...
    def execute(self, collage_id: str, new_name: str) -> Collage:
        ...
        Raises ValueError (from the aggregate) if *new_name* is blank.
        Raises LookupError if no Collage with that id exists.
        """
        collage = self._repository.find_by_id(collage_id)
        if collage is None:
            raise LookupError(f"No collage found with id={collage_id!r}.")
