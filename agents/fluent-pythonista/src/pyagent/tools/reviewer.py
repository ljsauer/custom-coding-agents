...
from pyagent.prompts import build_review_prompt
...
class ReviewTool:
    """Performs opinionated code reviews against the standards docs."""
    ...
    def description(self) -> str:
        return "Review Python code for correctness, style, and best practices."
    ...
    async def execute(
        ...
        *,
        ...
        """Run a code review on the provided source.

        Args:
            code: Python source code to review.
            filename: Optional filename for context.
            user_request: Additional review instructions.
            codebase: Optional broader codebase context.
            knowledge_base: Knowledge base for retrieving standards.

        Returns:
            A ``ToolResult`` containing the formatted review.
        """
        # Build RAG context from relevant docs.
        rag_context = ""
        if knowledge_base:
            query = f"code review {user_request or ''} {filename or ''}".strip()
        ...
        logger.info("Running review on %s (%d chars)", filename or "<stdin>", len(code))
